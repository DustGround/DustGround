...
class WaterParticle:
    """Individual water particle with flow properties"""
    def __init__(self, x: float, y: float):
        self.x = x
        self.y = y
        self.vx = 0.0
        self.vy = 0.0
        self.mass = 0.8  # Slightly lighter than sand
    ...
    def apply_viscosity(self, viscosity: float):
        """Apply viscosity/drag to movement"""
        self.vx *= (1 - viscosity)
        self.vy *= (1 - viscosity)
    ...
    def update(self, gravity: float, viscosity: float):
        ...
        # Cap falling speed (but slower cap than sand)
        if self.vy > 15:
            self.vy = 15


class WaterSystem:
    """Manages all water particles and their interactions"""
    def __init__(self, width: int, height: int):
        self.width = width
        self.height = height
        self.particles: List[WaterParticle] = []
        self.gravity = 0.15  # Slightly less than sand
        self.viscosity = 0.08
        self.cell_size = 3  # Larger cells reduce neighbor checks and improve perf
        self.grid = {}
    ...
    def _handle_collisions(self, frame_index: int = 0):
        """Handle water-to-water collisions with pressure"""
        ...
        for particle in self.particles:
            ...
            for other in neighbors:
                ...
                if 0.1 < dist < 2.5:
                    ...
                    # Pressure-based separation (more gentle than sand)
                    separation = 0.3
    ...
    def get_point_groups(self) -> Tuple[Tuple[int, int, int], List[Tuple[int, int]]]:
        """Return a single color and a list of (x, y) integer points for GPU batched drawing."""
        color = (100, 149, 237)
