...
def _project_root() -> str:
    # src/opt.py -> project root is its parent directory's parent
...
def _detect_gpu_available() -> bool:
    try:
        ...
        from pygame._sdl2 import rect as sdl2rect  # noqa: F401
        return True
    except Exception:
        return False
...
def _bench_gpu_points(size: Tuple[int, int], n_points: int = 40000, seconds: float = 0.35) -> float:
    ...
    # Create a separate SDL2 window that does not affect pygame.display()
    window = Window("bench", size=size)
    renderer = Renderer(window, vsync=False)
    clock = pygame.time.Clock()

    # Random points within the window
    w, h = size
    ...
    while time.perf_counter() - start < seconds:
        # Minimal pumping to keep OS happy without touching main display
        pygame.event.pump()
        ...
        renderer.present()
        frames += 1
        clock.tick()  # run uncapped during bench
...
def _bench_cpu_points(size: Tuple[int, int], n_points: int = 40000, seconds: float = 0.35) -> float:
    # Offscreen benchmark using a Surface to avoid interfering with the main window
    w, h = size
    ...
    while time.perf_counter() - start < seconds:
        surf.fill((20, 20, 20))
        # mimic our CPU fallback: draw small radius circles
        for (x, y) in pts:
...
def get_or_create_optimizations(game_size: Tuple[int, int]) -> Dict:
    ...
    # Run quick benches
    # Use the full game render size (no sidebar) for relevance
    ...
    # Target FPS heuristic
    target_fps = 120 if best_fps >= 115 else 60

    # Particle capacity heuristic: we benchmarked N points; scale by FPS vs target and keep margin
    n_points = 40000
    ...
    new_cfg = {
        "use_gpu": bool(best_is_gpu),
        "target_fps": int(target_fps),
        "max_particles": int(max_particles),
        "cpu_fps": float(round(cpu_fps, 2)),
        "gpu_fps": float(round(gpu_fps, 2)),
        "_meta": {
            "pygame_version": pygame_ver,
