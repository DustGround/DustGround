...
def discover_plugins(plugins_dir: Path) -> List[PluginInfo]:
    ...
    for zip_path in sorted(plugins_dir.glob('*.zip')):
        ...
        try:
            ...
            mtime = 0.0
        plugins.append(PluginInfo(id=pid, name=name, version=version, author=author, description=desc, zip_path=str(zip_path.resolve()), module=module, enabled=True, mtime=mtime))
    return plugins
