...
class PluginMenuPanel:
    def __init__(self):
        ...
        # simple timer for underline animation consistency
    ...
    def _compute_layout(self, surface_size: Tuple[int, int]) -> _Layout:
        ...
        # Responsive: list takes ~45%, detail takes rest
    ...
    def _compute_back_rect(self, surface_size: Tuple[int, int]) -> pygame.Rect:
        width, height = surface_size
        left_margin = 64
        # Match main menu 'Back' label sizing (use item_font, no panel)
    ...
    def _window_size(self) -> Tuple[int, int]:
        # Works both in CPU surface and SDL2 renderer modes
        try:
    ...
    def handle_event(self, event: pygame.event.Event) -> Optional[str]:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.button == 3:
                return "back"
            if event.button == 1:
                # Back button click (compute on-the-fly to be robust before draw)
                ...
                if layout.list_x <= mx < layout.list_x + layout.list_w and layout.list_y <= my < layout.list_y + layout.list_h:
                    ...
                    if 0 <= idx < len(plugins):
                        ...
                        # if click near right side inside row -> toggle
                ...
                if sel and layout.detail_x <= mx < layout.detail_x + layout.detail_w:
                    ...
                    if btn2.collidepoint(mx, my):
                        # Attempt to open plugins folder in system file explorer
                        try:
                            ...
                            pass
                    # Reload browser (just refresh service)
    ...
    def draw_cpu(self, screen: pygame.Surface) -> None:
        ...
        # Back label (match main menu style: plain text with underline)
        width, height = screen.get_size()
        rect = self._compute_back_rect((width, height))
        # advance timer for subtle pulse
    ...
    def draw_gpu(self, renderer, left_margin: int = 64) -> None:
        if Texture is None or sdl2rect is None:
            return
        # compute layout using output size
        try:
        ...
        if sel:
            ...
            y += auth_surf.get_height() + 12
            desc = sel.description or "No description provided."
            # naive wrap for GPU: draw as multiple CPU surfaces, copy as textures
            wrap_w = layout.detail_w - 16
            words = desc.split()
            line = ""
            while words:
        ...
        # underline (full width, accent; pulse optional omitted for simplicity)
