...
def recommend_settings(total_particles: int, fps: float, target_fps: int, gpu: bool) -> Dict:
	"""Return adaptive performance settings based on current load.

	The scaler reduces collision work when FPS drops below target
	and/or particle counts are high. Returns a dict with keys:
	  - sand: {neighbor_radius, max_neighbors, skip_mod}
	  - water: {neighbor_radius, max_neighbors, skip_mod}
	"""
	# Base tiers from FPS
	...
	# Particle pressure adds to tier (heavier budget reduction)
	if total_particles > 100_000:
		tier += 2
	elif total_particles > 60_000:
		tier += 1
	...
	# GPU typically handles draw cost; physics stays CPU-bound.
	# If GPU not available, be a bit more aggressive.
